...
class KuaishouBarrage(object):
    ...
    def extract_user_rank(self, data: str):
        ...
        for i in users:
            ...
            eid = self.unhexlify(uid_name[0])
            print("kwaiid：" + eid)
            name = self.unhexlify(uid_name[1][2:-2])
            print("昵称：" + name)
            avatar_url = self.unhexlify(page_url)
            print("头像：" + avatar_url)
    ...
    def extract_user_comment(self, data: str):
        ...
        for i in users:
            ...
            if len(uid_name_comment) == 2:
                eid = self.unhexlify(uid_name_comment[0])
                ...
                if len(name_comment[0]) % 2 == 0 and len(name_comment[1]) % 2 == 0:
                    name = self.unhexlify(name_comment[0])
                    comment = self.unhexlify(name_comment[1][0:])
                    print("eid：" + eid)
                    print("昵称：" + name)
                    print("评论：" + comment)
    ...
    def on_open(self, ws):
        ...
        def run():
            ...
            while Flag:
                ...
                heartbeat = head + self.hex(timestamp)
